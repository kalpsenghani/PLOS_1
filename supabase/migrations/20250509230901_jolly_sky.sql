/*
  ...
  3. Triggers
    - Add update_updated_at triggers with existence checks
*/
...
-- Update function (only create if it doesn't exist)
DO $$ 
BEGIN
...
-- Update triggers (only create if they don't exist)
DO $$
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_social_events_updated_at') THEN
